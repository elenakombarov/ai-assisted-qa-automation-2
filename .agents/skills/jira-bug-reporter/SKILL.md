...
Read tool schemas before calling (`mcps/plugin-atlassian-atlassian/tools/`).

Resolve `cloudId` via `getAccessibleAtlassianResources`, or pass the site hostname (e.g. `legionqaschool.atlassian.net`).

Search duplicates with `searchJiraIssuesUsingJql`.

Create the sub-task with `createJiraIssue`:
...
Confirm creation with `getJiraIssue` and return the new issue key and URL to the user.
